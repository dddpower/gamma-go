...
class Move():
    def __init__(self, point: Optional[Point]=None, is_pass=False, is_resign=False):
        ...
        self.point = Point(0, 0) if point is None else point
...
class Board():
    def __init__(self, num_rows: int, num_cols: int) -> None:
        ...
        self._hash: int = zobrist.EMPTY_BOARD

    def place_stone(self, player: Player, point: Point):
        assert self.is_on_grid(point)
        assert self._grid.get(point) is None

        # Compute the configuration of adjacent strings
        ...
        # Reduce liberties from adjacent strings of the opposite color
...
class GameState():
    ...
    def is_over(self):
        ...
        second_last_move = self.previous_state.last_move \
            if self.previous_state is not None else None
    ...
    def is_move_self_capture(self, player: Player, move: Move):
        ...
        return new_string.num_liberties == 0 if (new_string is not None) else True
